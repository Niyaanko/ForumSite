...
<?= form_open("admin/users");?>
            <table class="user_content">
                <tr>
                    ...
                    <td class="right_td"><?= html_escape($user_item['nickname']);?></td>
                </tr>
                <tr>
                    <th>メールアドレス</th>
                    <td><?= $user_item['mailaddress'];?></td>
                    <th class="right_th">権限</th>
                    <td class="right_td"><?= $user_item['permission'];?></td>
                </tr>
                <tr>
                    <th>コメント数</th>
                    <td><?= $user_item['comment_count'];?></td>
                    <th class="right_th">スレッド作成数</th>
                    <td class="right_td"><?= $user_item['threads_create_count'];?></td>
                </tr>
                <tr>
                    <th>通報されている数</th>
                    <td><?= $user_item['report_count'];?></td>
                    <th class="right_th">削除されたコメント数</th>
                    <td class="right_td"><?= $user_item['deleted_comment'];?></td>
                </tr>
                <tr class="row_link">
                    <td colspan="4">
<?php 
        if($user_item['permission'] === 'NORMAL'): ?>
                        <input type="hidden" name="ban_hdn" value="<?= $user_item['user_id']; ?>">
                        <input class="ban_btn" type="submit" name="submit" value="このユーザーをBAN">
<?php 
        elseif($user_item['permission'] === 'BANNED'): ?>
                        <input type="hidden" name="recover_hdn" value="<?= $user_item['user_id']; ?>">
